...
```
gdre_tools.exe --headless --recover="%input_path%/SlayTheSpire2.pck" --output="%output_dir%"
```
...
multiplayer
  // Steam and ENet (Godot's networking)
  // client-host model
```
...
// quite minimal, effective enough for the game, feels good

CombatState
  // all in all it's a blackboard memory
```
...
-- initialization --
  + IClientConnectionInitializer
    + ENetClientConnectionInitializer
      > Connect
        > create `ENetClient`
        > run `NetClientGameService -> Initialize`
        > run `ENetClient -> ConnectToHost`

    + SteamClientConnectionInitializer
      > Connect
        > create `SteamClient`
        > run `NetClientGameService -> Initialize`
        > run `SteamClient -> ConnectToLobbyOwnedByFriend or ConnectToLobby`

-- transport --
  + NetClient
    // `Update` pumps messages

    + ENetClient
      > _connection : ENetConnection
      > _peer : ENetPacketPeer
      > Update - packets include connection data or game messages
        > run `ENetConnection -> TryService`
        > handle events and packets
          // will call `INetHandler -> OnPacketReceived`
      > SendMessageToHost
        > run `ENetPacketPeer -> Send`

    + SteamClient
      > Update - only game messages
        > run `SteamUtil -> ProcessMessages`
      > SendMessageToHost
        // GC pin/unpin `byte[]`, because Steamworks is native
        > run `SteamNetworkingSockets -> SendMessageToConnection`
  ...
  + NetHost
    ...
    + ENetHost
      > _connection : ENetConnection
      > _connectedPeers : list of `ulong netId, ENetPacketPeer`
      > Update - packets include connection data or game messages
        > run `ENetConnection -> TryService`
        > handle events and packets
          // will call `INetHandler -> OnPacketReceived`
    ...
    + SteamHost
      > Update - only game messages
        > run `SteamUtil -> ProcessMessages` for each connection
      > SendMessageToClient
        // GC pin/unpin `byte[]`, because Steamworks is native
        > run `SteamNetworkingSockets -> SendMessageToConnection`
...
-- services --
  + INetGameService
    ...
    + NetClientGameService : INetHandler, INetGameService
      > _messageBus : NetMessageBus

      > Update
        > run `NetHost -> Update`

      > OnPacketReceived
        > run `NetMessageBus -> TryDeserializeMessage`
        > run `SendMessageToAllHandlers`

      > SendMessage
        > run `NetMessageBus -> SerializeMessage`
        > run `NetClient -> SendMessageToClient` for all clients

    + NetHostGameService : INetHandler, INetGameService
      > _messageBus : NetMessageBus

      > StartENetHost
        // create an start `ENetHost`

      > StartSteamHost
        // create an start `SteamHost`

      > Update
        > run `NetClient -> Update`

      > OnPacketReceived
        // same as `NetClientGameService` with additional
        // remote broadcast to all clients

      > SendMessage
        > run `NetMessageBus -> SerializeMessage`
        > run `NetClient -> SendMessageToHost`
...
-- serialization --
  NetMessageBus
    ...
    > SerializeMessage
      > `PacketWriter -> Reset`
      > write header bytes
      > run `INetMessage -> Serialize`
        > run multiple `PacketWriter -> Write*`
        > /voilà/

    > TryDeserializeMessage -> for `byte[]`
      > `PacketReader -> Reset`
      > read header bytes
      > `Activator -> CreateInstance`
      > run `INetMessage -> Deserialize`
        > run multiple `PacketReader -> Read*`
        > /voilà/

    > SendMessageToAllHandlers
      // local broadcast to all subscriptions

  PacketReader
    > _tempBuffer : `byte[16]`
      // staging buffer for primitives
    > _stringBuffer : `byte[64]`, grows when needed
      // staging buffer for strings
    > Buffer : `byte[]`
      // source buffer, that is written into staging, which is converted into values
    > Reset -> with `byte[]`
      > reset position, set current "buffer"
  ...
  PacketWriter
    ...
    > _stringBuffer : `byte[64]`, grows when needed
      // staging buffer for strings
    > Buffer : `byte[1024]`, grows when needed
      // target buffer, that is filled from staging, which is converted from values
    > Reset
      > reset position
...
-- ENet packet format
  // header
  0 : byte   - `ENetPacketType` (handshake request/response, disconnect, message)
  // message
  1 : ...

-- message format --
  // header
  0 : byte  - type (ordinal index from `NetTypeCache`)
  1 : ulong - sender ID
  // body
  5 : ...
  // @note bits are being writter and read, not bytes
  // as an optimization for booleans
```
...
ENetConnectionExtension
  > TryService -> for `ENetConnection`
    > run `ENetConnection -> Service`

SteamUtil
  > _messageBuffer : nint[64]
  > ProcessMessages
    > run `SteamNetworkingSockets -> ReceiveMessagesOnConnection`
    // for each one also run
    // - `INetHandler -> OnPacketReceived`
...
// there's a `ReactionType` enum, but game operates sprite frames first, and then
// deduces and id for it using `NReaction -> TextureToType`
```
