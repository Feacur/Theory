...
feels
  ...
  jurors' cards [quite] clearly convey mechanics; there are some biases in gameplay
  [neg] jurors' cards are shining, but "Go to trial" banner does not; a bit misleading

looks
  glorious ms paint art
  stylized cursor, camshake
  animations, shines, tweens
...
BenchArea drag'n'drop
```
basic parameters
  - board size (1D, 2D, etc)
  - maximum separation

rearranging is consisted of two parts
  1. remove from the array
     find possible index by visual positions
     insert back intro the array
  2. relax cards visual positions to match their index
     currently dragged card is a part of the array at this point

there's also a possibility to drop a juror into the selling area

// probably, it would be possible to limit removal and insertions
// with only two instances: drag started, drag ended; relaxation code
// then should offset by 1 anything to the right of the `dragndrop_index`.
// it's less about memory, because backing array of a list keeps its size, but
// multiple unnecessary items rearrangements. not a problem at that scale, though

// should the game have a rearrangeable drop area like in hearthstone
// `dragnrop_index` approach becomes more crucial, with an alternative
// of inserting a temporrary null or a dummy item until the "drop" event

// anyway, this code is concise and effective; being all-in with enterprise
// slutions would be a for more horrible experience
```
...
coroutines are heavily used for sequencing

some data, some hardcode

arrays actively check and remove nulls instead of per-event actions only

generally speaking, events are being used, but a lot of places just do thing on tick

data is in the `Resource` folder; again, which makes sense for a demo

it's a demo, there's no save sytem, ofc; progress is stored in statics

if you need 2D, please, use sprite, be a good friend to yourself

use of layers and layer offsets (useful for cursor or for drag'n'drop)
```
